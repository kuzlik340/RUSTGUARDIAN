...
1. **Engine** – In SafeConnection mode, this module monitors all USB device connections and disconnections, comparing each device against a whitelist. If a device is not on the whitelist, the engine will check all files on the device (if it is a flash drive) for viruses. In LockDown mode engine will scan processes that are currently running if they look strange. Also the Lockdown mode implements a crucial part of the project — the keylogger, along with all the necessary modules required for its functionality. Essentially, it can detect and intercept a BadUSB attack and log all keystrokes generated by the malicious device.
